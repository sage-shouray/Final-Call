...
async def run_fb60_direct(document_id: str, form_data: dict[str, Any], posted_by: str = "system") -> None:
    ...
    async with AsyncSessionLocal() as session:
        ...
        try:
            ...
            await doc_repo.update_fb60_posting(doc_id, posting_data)
            await session.commit()
            bound_log.info("FB60 posting saved", fb60_number=fb60_resp.fb60_number)

        except Exception as exc:
            bound_log.error("FB60 posting failed", error=str(exc), traceback=_tb.format_exc())
            error_entry = {
                "stage": "fb60_posting", "message": str(exc) or type(exc).__name__,
                "detail": type(exc).__name__, "timestamp": datetime.now(UTC).isoformat(),
            }
            try:
                doc2 = await doc_repo.find_by_document_id(document_id)
                if doc2:
                    await doc_repo.update_status(doc2["id"], DocumentStatus.EXTRACTED, error_entry=error_entry)
                    await session.commit()
            except Exception:
                pass
